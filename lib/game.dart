...
class _GameState extends State<Game> {
  ...
  Future<void> _initializeGame() async {
    ...
    if (validWords.isEmpty) {
      ...
      print("Nenhuma palavra encontrada em lexico.txt com o tamanho ${widget.wordLength}");
      return;
    }
  ...
  Future<bool> _checkWordOnline(String word) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('https://api.dicionario-aberto.net/word/${word.toLowerCase()}'),
      );
  ...
  Future<void> _submitGuess(String value) async {
    ...
    if (value.length != widget.wordLength) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('A palavra deve ter ${widget.wordLength} letras.')));
      return;
    }
    ...
    if (isLocallyValid) {
      ...
      if (isOnlineValid) {
        ...
        setState(() { _dictionary.add(value); });
        _processValidGuess(value);
      } else {
        // acho q n existe ent né
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Palavra não encontrada no dicionário.')));
      }
    }
  }
  ...
  void _processValidGuess(String value) {
    setState(() {
      ...
      for (int i = 0; i < value.length; i++) {
        ...
        final char = value[i];
        if (_secretWord.contains(char)) { bgColor = Colors.amber.shade700; }
        if (_secretWord[i] == char) { bgColor = Colors.green.shade700; }
        ...
        letters.add(
          Padding(
            ...
            child: Container(
              ...
              child: Text(value[i], style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold)),
            ),
          ),
        );
      }
      // composição das letras que formam uma palavra/tentativa
      _attempts.add(Row(mainAxisAlignment: MainAxisAlignment.center, children: letters));
  ...
  void _showLoadingDialog(String message) {
     showDialog(context: context, barrierDismissible: false, builder: (BuildContext context) {
        return AlertDialog(content: Row(children: [const CircularProgressIndicator(), const SizedBox(width: 24), Text(message)]));
    });
  }
  ...
  void _showEndGameDialog(String title, String content) {
     showDialog(context: context, barrierDismissible: false, builder: (BuildContext context) {
        return AlertDialog(title: Text(title), content: Text(content), actions: <Widget>[
            TextButton(child: const Text('Jogar Novamente'), onPressed: () {
                Navigator.of(context).pop();
                Navigator.of(context).pop();
            })
        ]);
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(appBar: AppBar(title: const Text("LETRADLE")), body: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(), SizedBox(height: 16), Text("Carregando dicionário...")])));
    }
    
    return Scaffold(appBar: AppBar(title: const Text("LETRADLE")), body: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [
        Expanded(child: ListView.builder(itemCount: _attempts.length, itemBuilder: (context, index) { return _attempts[index]; })),
        const SizedBox(height: 16),
        TextField(controller: _attemptControler, decoration: const InputDecoration(labelText: 'Sua tentativa', border: OutlineInputBorder()), maxLength: widget.wordLength, textAlign: TextAlign.center, style: const TextStyle(fontSize: 22, letterSpacing: 4), onSubmitted: _submitGuess)
    ])));
  }
}